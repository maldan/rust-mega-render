...
// hair, not the head behind it. Vertex skinning matches mesh.wgsl (bone_tex).
...
struct ObjectUniforms {
    ...
    // x = metallic (unused), y = roughness, z = skin mode (0/LBS/DQS), w = cutout fringe
    params: vec4<f32>,
...
struct VertexOutput {
    ...
    @location(4) uv_q: f32,
    // Per-strand shade (color.y) and layer opacity (color.z).
...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    let q = max(in.color.x, 1e-5);
    out.uv = in.uv * q;
    out.uv_q = q;
    out.shade_alpha = vec2<f32>(in.color.y, in.color.z);
