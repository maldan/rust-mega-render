//! Temporal resolve for DOF — like SSR temporal, but keeps sky (cubemap) in the filter.
...
fn fs(i: VsOut) -> @location(0) vec4<f32> {
    ...
    if !sky {
        ...
        if depth_err > reject || (hist_depth >= 0.9999 && depth < 0.999) {
            return vec4(cur, out_depth);
        }
        ...
        let pad = (n_max - n_min) * 0.2 + vec3(0.003);
        hist = vec4(clamp(hist.rgb, n_min - pad, n_max + pad), hist.a);
        ...
        return vec4(mix(cur, hist.rgb, w_hist), out_depth);
    }

    // Sky: softer history (cubemap moves with camera rotation via reprojection).
    // Reject if history wasn't sky.
    if hist_depth < 0.999 {
        return vec4(cur, out_depth);
    }
    let w_sky = clamp(u.params.x, 0.0, 0.95);
    let dims = vec2<f32>(textureDimensions(current_tex, 0));
    let texel = 1.0 / dims;
    var n_min = cur;
    var n_max = cur;
    ...
    let pad = (n_max - n_min) * 0.25 + vec3(0.004);
    let h = clamp(hist.rgb, n_min - pad, n_max + pad);
    return vec4(mix(cur, h, w_sky), out_depth);
}
